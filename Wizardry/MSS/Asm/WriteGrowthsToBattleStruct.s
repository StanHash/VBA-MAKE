...
.org 0 @ not necessary if 0, but I put it anyway out of habit

@ setting up the literal pool
pGetHpGrowth  = Class_Level_Cap_Table+4
pGetPowGrowth = Class_Level_Cap_Table+8
pGetSklGrowth = Class_Level_Cap_Table+12
pGetSpdGrowth = Class_Level_Cap_Table+16
pGetDefGrowth = Class_Level_Cap_Table+20
pGetResGrowth = Class_Level_Cap_Table+24
pGetLukGrowth = Class_Level_Cap_Table+28
GrowthOptions = Class_Level_Cap_Table+32
...
ldr r0,GrowthOptions @ bit 1 is set if we should check for fixed growths mode
mov r1,#0x1
...
HpGrowth:
ldr r0,pGetHpGrowth
mov r14,r0 @ use the longcalling trick to bl to pGetHpGrowth, whose location will be determined in the EA file

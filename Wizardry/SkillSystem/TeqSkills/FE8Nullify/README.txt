...
Class weaknesses use a word at +0x50 (currently unused). This is set via the class data editor csv, or whatever way you edit tables. The definitions (EFF_ARMOR, EFF_HORSE, etc) are in Tables/Table Definitions.txt. Class weaknesses have been filled in for existing classes in the csv because that isn't used, while effectiveness data is set in Nullify EA.txt
...
Example: The rapier does 3x damage to armor and cavalry types. Therefore, the rapier list would look like SetWeaponEffectiveness(EFF_ARMOR|CavalryType, 6).
...
Example: SetSlayerTableEntry(Bishop, EFF_MONSTER, 6)
